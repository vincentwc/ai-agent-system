...
from langchain_core.runnables import RunnablePassthrough  # 添加此行导入
...
# langchain中的向量存储对象，有一个方法as_retriever，可以返回一个runmable接口的子类对象
...
retriever
  - 输入：用户的提问   str
  - 输出：向量库检索到的结果 list[Document]

prompt
  - 输入：用户的提问，向量库检索到的结果 dict
